...
# ---------------------------------------------------------------
# Copyright (c) 2022, SegNeXt Authors. All rights reserved.
#
# Licensed under the Apache-2.0 Source Code License. For full license
# terms, please refer to the LICENSE file provided with this code
# or visit the SegNeXt source repository at
...
class StemConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.proj = nn.Sequential(
            ...
            nn.SyncBatchNorm(num_features=out_channels // 2),
            nn.GELU(),
            nn.Conv2d(
                out_channels // 2,
                out_channels,
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4.0, drop=0.0, drop_path=0.0, act_layer=nn.GELU):
        super().__init__()
        self.norm1 = nn.SyncBatchNorm(num_features=dim)
        self.attn = SpatialAttention(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = nn.SyncBatchNorm(num_features=dim)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.norm = nn.SyncBatchNorm(num_features=embed_dim)
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                fan_out //= m.groups
                torch.nn.init.normal_(m, mean=0, std=math.sqrt(2.0 / fan_out))
...
mscan_encoders = {
    "mscan_t": {
        "encoder": MSCAN_Encoder,
        "params": {
            "out_channels": [3, 32, 64, 160, 256],
            "mlp_ratios": [4, 4, 4, 4],
            "depths": [2, 2, 2, 2],
        },
    },
